...
class RegistrationVC: UIViewController {
    ...
    @objc private func handleSignUpButton() {
        guard let profileImage = addImageButton.imageView?.image else {  return  }
        guard let email = emailTF.text else {  return  }
        guard let password = passwordTF.text else { return }
        guard let userName = userNameTF.text else { return }
        self.displayActivityIndicator(shouldDisplay: true)
        FireBaseStore.createUser(email: email, password: password) { (error) in
            if error != nil {
            ...
            FireBaseStore.updateProfileInfo(withImage: imageData, name: userName) { (error) in
                if error != nil {
                    self.presentAlertWithTitle(title: "Error", message: error?.localizedDescription ?? "Unknown Error.", options: "Ok") { (option) in
                        print("option: \(option)") }
                    self.displayActivityIndicator(shouldDisplay: false)
...
extension RegistrationVC: ImagePickerDelegate {
    func didSelect(image: UIImage?) {
        ...
        self.addImageButton.layer.borderWidth = 1.5
        self.addImageButton.layer.borderColor = UIColor.black.cgColor
